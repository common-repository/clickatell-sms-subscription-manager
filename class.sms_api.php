...
class sms {
    ...
    * @access private
    */
    function _auth($apikey,$user,$pass) {
    	$comm = sprintf ("%s/auth?api_id=%s&user=%s&password=%s", $this->base_s, $apikey, $user, $pass);
    ...
    */
    function _parse_send ($result) {
    	$code = substr($result, 0, 2);
    	if ($code!="ID") {
			//Do nothing, send error through to frontend.
